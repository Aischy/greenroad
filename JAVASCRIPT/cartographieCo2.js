...
function colorMarkerCo2(donnee, type) {
    let icon;
    if (type == 'son') {
        if (donnee>=85) {
            icon = co2violet ;
...
function getData(){
    ...
    xmlHttpRequest.open("GET", "../PHP/apiCo2.php", true);
    xmlHttpRequest.send();
    ...
    xmlHttpRequest.onreadystatechange = function(){
        if (xmlHttpRequest.status === 200 && xmlHttpRequest.readyState === 4) {
            ...
            for (var key in response_json) {
                ...
                L.marker([response_json[key].latitude, response_json[key].longitude], {icon: colorMarkerCo2(parseInt(response_json[key].donnee),response_json[key].type)}).addTo(map).bindPopup("I am the key."+ key);
